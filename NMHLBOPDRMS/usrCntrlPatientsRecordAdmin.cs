...
{
    public partial class usrCntrlPatientsRecordAdmin : UserControl
    {
        MySQL_Code conStr = new MySQL_Code();
        public MySqlConnection conn = new MySqlConnection(MySQL_Code.connectionString);

        public usrCntrlPatientsRecordAdmin()
        {
            InitializeComponent();
            autoCompleteLastname();
        }

        private void usrCntrlPatientsRecordAdmin_Load(object sender, EventArgs e)
        {
            getDataSet();
        ...
        private void dgPatientsRecord_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        {
            conn.Open();
            string sql = "SELECT COUNT(patientsNo) AS 'Patients Number', lastName AS 'Lastname' , firstName AS 'Firstname' , middleName AS 'Middlename' FROM tblPatients_Info WHERE lastName LIKE  '" + txtSearch.Text + "' OR firstName LIKE  '" + txtSearch.Text + "' OR middleName LIKE  '" + txtSearch.Text + "' ";
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            MySqlDataAdapter adapter = new MySqlDataAdapter(sql, conn);
            DataSet dS = new DataSet();
            Int32 count = Convert.ToInt32(cmd.ExecuteScalar());
