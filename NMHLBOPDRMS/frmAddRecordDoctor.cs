...
namespace NMHLBOPDRMS
{
    public partial class frmAddRecordDoctor : Form
    {
        ...
        public void oldRecordMedicalDiagnosis()
        {
            if (conn.State == ConnectionState.Open)
                conn.Close();
            conn.Open();
            string sql = "SELECT * FROM tblMedical_Diagnosis WHERE patientsNo = @patientnum";
            ...
            while (reader.Read())
            {
                ...
                doctor = reader.GetString("medical_doctor");
                status = reader.GetString("status");
                admissionStat = reader.GetString("admissionStatus");
                diagnosis = reader.GetString("diagnosis");
                notes = reader.GetString("notes");
                date = reader.GetDateTime("date").ToString("yyyy-MM-dd");
                lstOld.Items.Add(diagnosis);
            }
            conn.Close();
            ...
            {
                conn.Open();
                string sql1 = "INSERT INTO tblOld_Medical_Diagnosis VALUES (@patientnum, @doctor, @status, @admissionStat, @diagnosis, @notes, @date); DELETE FROM tblMedical_Diagnosis WHERE patientsNo = @patientnum";
                MySqlCommand cmd1 = new MySqlCommand(sql1, conn);
                cmd1.Parameters.AddWithValue("@patientnum", patientnum);
                cmd1.Parameters.AddWithValue("@doctor", doctor);
                cmd1.Parameters.AddWithValue("@status", status);
                cmd1.Parameters.AddWithValue("@admissionStat", admissionStat);
                cmd1.Parameters.AddWithValue("@diagnosis", lstOld.Items[a].ToString());
        ...
        public void oldRecordTreatment()
        {
            if (conn.State == ConnectionState.Open)
                conn.Close();
            conn.Open();
            string sql = "SELECT * FROM tblMedical_Treatment WHERE patientsNo = @patientnum";
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            cmd.Parameters.AddWithValue("@patientnum", lblNumber.Text);
            MySqlDataReader reader = cmd.ExecuteReader();
            int x = 0;
            ...
            while (reader.Read())
            {
                ...
                medical_doctor = reader.GetString("medical_doctor");
                date = reader.GetDateTime("date").ToString("yyyy-MM-dd"); ;
            }
            conn.Close();
            ...
            {
                conn.Open();
                string sql1 = "INSERT INTO tblOld_Medical_Treatment VALUES (@patientnum, @treatment, @medical_doctor, @date); DELETE FROM tblMedical_Treatment WHERE patientsNo = @patientnum";
                MySqlCommand cmd1 = new MySqlCommand(sql1, conn);
                cmd1.Parameters.AddWithValue("@patientnum", patientnum);
                cmd1.Parameters.AddWithValue("@treatment", treatment);
                cmd1.Parameters.AddWithValue("@medical_doctor", medical_doctor);
                cmd1.Parameters.AddWithValue("@date", date);
                cmd1.ExecuteNonQuery();
                conn.Close();
            }
        ...
        public void submitTreatment()
        {
            ...
            MessageBox.Show("Medical Treatment succesfully saved.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnSubmitTreatment_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Are you sure you want to save?", "Question", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                {
                    MessageBox.Show("Please enter Treatment for Patient", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtTreatment.Focus();
                }
            }
        }
        ...
        private void frmAddRecordDoctor_Load(object sender, EventArgs e)
        {
            string time = "";
