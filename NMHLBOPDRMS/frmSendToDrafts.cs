...
{
    public partial class frmSendToDrafts : Form
    {
        MySQL_Code conStr = new MySQL_Code();
        public MySqlConnection conn = new MySqlConnection(MySQL_Code.connectionString);

        public frmSendToDrafts()
        {
            InitializeComponent();
        }

        private void frmSendToDrafts_Load(object sender, EventArgs e)
        {
            conn.Open();
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            if (cboDoctorsName.Text != "")
            {
                if (conn.State == ConnectionState.Open)
                    conn.Close();
                conn.Open();
                string sql = "UPDATE tblDrafts_Nurse SET sentTo = @lname WHERE patientNo = @num; UPDATE tblNext_in_Line_Doctor SET sentTo = @lname WHERE patientNo = @num";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@num", number);
                cmd.Parameters.AddWithValue("@lname", cboDoctorsName.Text);
                cmd.ExecuteNonQuery();
                conn.Close();

                conn.Open();
                string s = "SELECT * FROM tblDrafts_Nurse WHERE patientNo = @nu";
                MySqlCommand cmm = new MySqlCommand(s, conn);
                cmm.Parameters.AddWithValue("@nu", number);
                string status = "";
                string requested = "";
                string examined = "";
                MySqlDataReader re = cmm.ExecuteReader();
                while (re.Read())
                {
                    status = re.GetString("status");
                    requested = re.GetString("datetequested");
                    examined = re.GetString("dateexamined");
                }
                conn.Close();
                ...
                conn.Open();
                string sq = "INSERT INTO tblnext_in_line_doctor VALUES (@num, @patientNo, @lname, @fname, @mname, @status, @daterequested, @dateexamined, @sentby, @sentto)";
                ...
                cm.Parameters.AddWithValue("@daterequested", requested);
                cm.Parameters.AddWithValue("@dateexamined", examined);
